...
function extractCommandWithValue(
  args: string[],
  command: string
): string | null {
...
export function extractArgs(inputArgs: string[]) {
  ...
  const onEmitDebounceMs =
    Number(extractCommandWithValue(args, "--onEmitDebounceMs")) || 300;
  ...
  const requestedToListEmittedFiles = extractCommand(
    args,
    "--listEmittedFiles"
  );
